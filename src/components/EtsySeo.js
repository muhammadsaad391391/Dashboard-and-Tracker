...
export function renderEtsySeo(container, state) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="padding: 12px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap;">
      <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        <button class="btn btn-secondary btn-sm" id="etsy-prev-week-btn">◀ Prev Week</button>
        <span style="font-size: 13px; font-weight: 700; color: var(--text-primary); font-family: var(--font-header);" id="etsy-week-range">${weekRangeText}</span>
        <button class="btn btn-secondary btn-sm" id="etsy-next-week-btn">Next Week ▶</button>
        <button class="btn btn-secondary btn-sm" id="etsy-today-btn" style="margin-left: 6px;">Today</button>
      </div>
  ...
  container.querySelector('#etsy-prev-week-btn').addEventListener('click', async () => {
  ...
  container.querySelector('#etsy-next-week-btn').addEventListener('click', async () => {
    const current = new Date(state.getActiveDate());
    current.setDate(current.getDate() + 7);
    const y = current.getFullYear();
    const m = String(current.getMonth() + 1).padStart(2, '0');
  ...
  container.querySelector('#etsy-today-btn').addEventListener('click', async () => {
  ...
  if (spreadsheetContainer) {
    if (savedScrollLeft > 0) {
      ...
      setTimeout(() => {
        ...
        const activeHeader = container.querySelector(`[id="etsy-header-${activeDate}"]`);
